...
// Type for the response expected from our proxy server calling OpenAI
// Proxy should return { langCode: ["translated text 1", "translated text 2", ...], ... }
...
async function callProxyServerForTranslations(textsToTranslate: OriginalTextItem[]): Promise<TranslationsByLang> {
        ...
        if (!proxyResponse.ok) {
            ...
            try {
                ...
                errorMsg = errorData?.error || errorMsg;
                 ...
                 const textError = await proxyResponse.text();
...
figma.ui.onmessage = async (msg: {type: string, apiKey?: string}) => { // apiKey still here from older message type, but not used
  ...
  if (msg.type === 'start-translation-process') {
    ...
    const translationsByLang = await callProxyServerForTranslations(originalTextData);
...
async function applyTranslationsToFrames(translationsByLang: TranslationsByLang) {
    ...
    originalTextNodes.forEach(node => { if (typeof node.fontName === 'symbol') { node.getStyledTextSegments(['fontName']).forEach((s: any) => fontsToLoad.add(s.fontName)); } else { fontsToLoad.add(node.fontName as FontName); }});
    try {
    ...
    } catch (e) { const err = e instanceof Error ? e.message : String(e); console.error("[Code] Error loading fonts:", e); figma.notify(`❌ Font Error: ${err}`, { error: true }); figma.ui.postMessage({ type: 'show-status', message: `Font Error: ${err}`, isError: true }); return; }
    ...
    for (const langCode of TARGET_LANGUAGES) {
        ...
        const hasErrors = !translatedTexts || translatedTexts.length === 0 || translatedTexts.some(
            t => t.text.startsWith('[API/Network Err') || t.text.startsWith('[Proxy') || t.text.startsWith('[DeepL') || t.text.startsWith('[OpenAI')
        );
        ...
        if (newTextNodes.length !== originalTextNodes.length) { console.error(`[Code] Text node mismatch ${langCode}`); newFrame.remove(); continue; }
        ...
        for (let i = 0; i < originalTextNodes.length; i++) {
            ...
            if (translatedText !== undefined && !translatedText.startsWith('[')) { // Only apply if not an error/placeholder? Your choice. Let's apply placeholders too now.
                 try {
                    ...
                    const nodeFontsToLoad = new Set<FontName>(); if (typeof newNode.fontName === 'symbol') { newNode.getStyledTextSegments(['fontName']).forEach((s: any) => nodeFontsToLoad.add(s.fontName)); } else { nodeFontsToLoad.add(newNode.fontName as FontName); }
                    ...
                    const fontErr = e instanceof Error ? e.message : String(e); console.error(`[Code] Font/Set error node ${i} lang ${langCode}: ${fontErr}`); try { await figma.loadFontAsync({ family: "Inter", style: "Regular" }); newNode.characters = '[Font Err]'; } catch (e2) {}
                }
            } else if (translatedText !== undefined && translatedText.startsWith('[')) {
                 // Apply placeholder/error text
                 try { await figma.loadFontAsync({ family: "Inter", style: "Regular" }); newNode.characters = translatedText; } catch (e) {}
     ...
     // --- Final Status Update ---
    if (framesCreated > 0) { figma.notify(`✅ Translation complete! ${framesCreated} frames created.`, { timeout: 5000 }); figma.ui.postMessage({ type: 'show-status', message: 'Done!' }); }
    else { figma.notify(`ℹ️ Translation finished, but no valid frames created. Check logs/API key.`, { timeout: 5000 }); figma.ui.postMessage({ type: 'show-status', message: 'Finished. No frames created (check logs).' }); }
